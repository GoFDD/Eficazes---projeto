  ...
  <main class="registro">
    <nav>
      <div class="flex mt-7 mb-8 border-b">
        ...
        >
          <b>Meu perfil</b>
          <span
            class="absolute bottom-0 left-0 w-full h-[4px] bg-gradient-to-r from-[#03B1FF] to-[#97C4D8]"
          ></span>
        </p>
      </div>
    </nav>
      ...
      <div class="col-span-4 flex flex-col space-y-3">
        <div class="flex items-center justify-between">
          <h3 class="nomecompleto__idade">
            {{ editData.nome }},
            {{ calculateAge(editData.dataNascimento) }} anos
          </h3>
          <div class="flex space-x-2">
            <button @click="goToHomePage" class="button">
              Voltar à Página Inicial
            </button>
            <button @click="showEditModal = true" class="button">
              Editar Perfil
            </button>
          </div>
        </div>
        ...
        <div
          class="dados__pessoais grid grid-cols-2 gap-4 p-3 border rounded-lg shadow-md bg-white"
        >
        ...
        <button @click="showModal = true" class="button w-auto mt-4">
          Ver todos endereços
        </button>
      </div>
    </div>
    ...
    >
      <div v-for="(address, index) in addresses.slice(1)" :key="index">
        ...
        <p>
          <strong>Número da Residência:</strong> {{ address.numeroResidencia }}
        </p>
      </div>
    </div>
      ...
      >
        <div class="modal-content bg-white rounded-lg w-11/12 md:w-1/2 p-5">
            ...
            <form @submit.prevent="saveAddressChanges">
              <div class="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.nomeRua"
                    class="input"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.bairro"
                    class="input"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.complemento"
                    class="input"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.cidade"
                    class="input"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.cep"
                    class="input"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    v-model="editingAddressData.numeroResidencia"
                    class="input"
                  />
                </div>
              </div>
              <button
                type="button"
                @click="cancelEdit"
                class="button mt-4 w-full bg-gray-500"
              >
                Cancelar
              </button>
              <button type="submit" class="button mt-2 w-full bg-blue-500">
                Salvar Alterações
              </button>
            </form>
          </div>
          <div v-else>
            <div class="space-y-4">
              ...
              >
                ...
                <p><strong>Cidade:</strong> {{ address.cidade }}</p>
                <p><strong>CEP:</strong> {{ address.cep }}</p>
                <button @click="editAddress(index)" class="button mt-2">
                  Editar
                </button>
              </div>
            </div>
          ...
          <button
            ...
            class="button mt-4 w-full bg-red-500"
          >
            Fechar
          </button>
        </div>
